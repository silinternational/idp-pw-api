<?php
use common\helpers\Utils;
return [
    'user1' => [
        'id' => 1,
        'uuid' => 'ab5489ea-48f3-4b37-b832-bcb3e95625a4',
        'employee_id' => '111111',
        'first_name' => 'User',
        'last_name' => 'One',
        'idp_username' => 'first_last',
        'email' => 'first_last@organization.org',
        'created' => '2016-02-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user1'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'login',
        'hide' => 'no',
    ],
    'user2' => [
        'id' => 2,
        'uuid' => 'e615784c-8646-4bc5-8a2b-ba087ca29277',
        'employee_id' => '222222',
        'first_name' => 'User',
        'last_name' => 'Two',
        'idp_username' => 'user_two',
        'email' => 'user_two@organization.org',
        'created' => '2016-02-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user2'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'login',
        'hide' => 'yes',
    ],
    'user3' => [
        'id' => 3,
        'uuid' => '77c647e8-a622-4917-a748-e1bf16ac6c3f',
        'employee_id' => '333333',
        'first_name' => 'User',
        'last_name' => 'Three',
        'idp_username' => 'user_three',
        'email' => 'user_three@organization.org',
        'created' => '2016-02-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user3'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'login',
        'hide' => 'no',
    ],
    'user4' => [
        'id' => 4,
        'uuid' => '5bf9225b-8f95-4195-9998-bb76f3364d3b',
        'employee_id' => '444444',
        'first_name' => 'User',
        'last_name' => 'Four',
        'idp_username' => 'user_four',
        'email' => 'user_four@organization.org',
        'created' => '2016-02-29 11:58:00',
        'auth_type' => null,
        'hide' => 'no',
    ],
    'user5' => [
        'id' => 5,
        'uuid' => '0bba13fe-d4ec-40be-baaa-ee07ce1d9438',
        'employee_id' => '5',
        'first_name' => 'User',
        'last_name' => 'Five',
        'idp_username' => 'user_five',
        'email' => 'user_five@organization.org',
        'created' => '2016-02-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user5'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'reset',
        'hide' => 'no',
    ],
    'user6' => [
        'id' => 6,
        'uuid' => '66ba13fe-d4ec-40be-baaa-ee07ce1d9438',
        'employee_id' => '6',
        'first_name' => 'Us',
        'last_name' => 'Sx',
        'idp_username' => 'us_sx',
        'email' => 'us_sx@organization.org',
        'created' => '2016-03-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user6'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'reset',
        'hide' => 'no',
    ],
    'user7' => [
        'id' => 7,
        'uuid' => '94b055f6-c0a8-4bcd-8e0a-e63ebd1b796f',
        'employee_id' => '7',
        'first_name' => 'User',
        'last_name' => 'Seven',
        'idp_username' => 'user_seven',
        'email' => 'user_seven@organization.org',
        'created' => '2016-03-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user7'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'reset',
        'hide' => 'no',
    ],
    'user8' => [
        'id' => 8,
        'uuid' => '08805a38-be74-45dc-9e3d-df1143d96f0d',
        'employee_id' => '8',
        'first_name' => 'User',
        'last_name' => 'Eight',
        'idp_username' => 'user_eight',
        'email' => 'user_eight@organization.org',
        'created' => '2016-03-29 11:58:00',
        'access_token' => Utils::getAccessTokenHash('user8'),
        'access_token_expiration' => Utils::getDatetime(time() + 1800),
        'auth_type' => 'reset',
        'hide' => 'no',
    ],
];
